		...
		if self.file_error_flag:
			with open(fileName, "r") as f:
				for i,l in enumerate([b for b in [a.replace('\n', '') for a in f.readlines()] if b != '']):
					...
					row = [b.strip() for b in [a for a in l.split(self.__comma) if a!='']]
					lenght = len(row)

					### listValues est vide, liste de temps remplit avec entiers dependant de la frÃƒÂ©quence de sortie
					if self.list_empty_flag :
						t = i*self.outPutFrequency
					### listValues non vide alors la colonne de temps est spÃƒÂ©cifier par time
					else:
						t = row[self.time]
					...
					if self.list_empty_flag:
						...
						for val in list(self.V.keys()):
							### si l'utilsiateur demande une colonne superieur au nombre de colonne dans le fichier -> derniere selectionnÃƒÂ©e
							if val >= lenght:
								self.V[val].append(row[lenght-1].strip())
							### l'utilisateur demande une colonne inferieur a 0 ou egale a 0 (colonne de temps) -> premiere selectionnÃƒÂ©e
							elif val < 0 or val == 0:
